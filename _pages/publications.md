---
layout: splash
permalink: /publications/
title: "Publications"
excerpt: "Published work "
header:
  overlay_color: "#5e616c"
  overlay_image: /assets/images/main_hero.png
---

<div>
<p style="margin:0;display:inline;float:left"><font size="3">&#8224; : Co-First author, &#x2a; : Co-Corresponding author</font></p>

<p style="margin:0;display:inline;float:right">
        <a href="#2025" class="btn btn--info btn--small">Published 2025</a>
        <a href="#2024" class="btn btn--info btn--small">Published 2024</a>
        <a href="#2023" class="btn btn--info btn--small">Published 2023</a>
        <a href="#2022" class="btn btn--info btn--small">Published 2022</a>
        <a href="#2021" class="btn btn--info btn--small">Published 2021</a>
        <a href="#2020" class="btn btn--info btn--small">Published 2020</a>
</p>
</div>
<br >

## 2025

{% include publication 
        authors="*Terstege, D.J.* & Epp, J.R."
        title="Environmental enrichment preserves retrosplenial parvalbumin density and cognitive function in female 5xFAD mice" 
        journal="Journal of Neuroscience" 
        page="2025;46(9):e0455252026" 
        pmid="41633836" 
        doi="10.1523/JNEUROSCI.0455-25.2026"                
%}

{% include publication 
        authors="Bardnard, I.L., McElroy, D.L., Young, K.M., *Terstege, D.J.*, Glass, A.E., Epp, J.R., Botterill, J.J., Howland, J.G."
        title="Activity of CaMKII-expressing neurons in medial prefrontal cortex of male and female Long Evans rats is necessary for encoding odor information and novelty recognition in an odor-based incidental memory test" 
        journal="Cerebral Cortex" 
        page="2025;35(8):bhaf203" 
        pmid="40751661" 
        doi="10.1093/cercor/bhaf203"        
%}

{% include publication 
        authors="O’Neill, O.S.<sup>&#8224;</sup>, *Terstege, D.J.*<sup>&#8224;</sup>, Gill, A.K., Edge-Partington, M., Ramkumar, R., Epp, J.R., Sargin, D."
        title="An open-source rodent limited bedding and nesting apparatus for chronic early life stress" 
        journal="eNeuro" 
        page="2025;12(6):ENEURO.0081-25.2025." 
        pmid="40555522" 
        doi="10.1523/ENEURO.0081-25.2025"        
%}
{% include publication 
        authors="*Terstege, D.J.*, Ren, Y., Ahn, B.Y., Seo, H., Adigun, K., Alzheimer's Disease Neuroimaging Initiative, Galea, L.A.M., Sargin, D., Epp, J.R."
        title="Impaired parvalbumin interneurons in the retrosplenial cortex as the cause of sex-dependent vulnerability in Alzheimer’s disease" 
        journal="Science Advances" 
        page="2025;11(18):eadt8976" 
        pmid="40305608" 
        doi="10.1126/sciadv.adt8976"        
%}
{% include publication 
        authors="*Terstege, D.J.*, Jabeen, S., Alzheimer’s Disease Neuroimaging Initiative, Galea, L.A.M., Epp, J.R., Sargin, D."
        title="SSRIs reduce plasma tau and restore dorsal raphe metabolism in Alzheimer's disease" 
        journal="Alzheimer's and Dementia " 
        page="2025;21(2):e14579." 
        pmid="39935329" 
        doi="10.1002/alz.14579"        
%}

## 2024

{% include publication 
        authors="*Terstege, D.J.*, Alzheimer’s Disease Neuroimaging Initiative, Galea, L.A.M., Epp, J.R."
        title="Retrosplenial hypometabolism precedes the conversion from mild cognitive impairment to Alzheimer’s disease" 
        journal="Alzheimer's and Dementia" 
        page="2024;20(12):8979-8986" 
        pmid="39470016" 
        doi="10.1080/19490976.2024.2351478"        
%}

{% include publication 
        authors="*Terstege, D.J.*, Goonetilleke, D., Barha, C.K, Epp, J.R."
        title="Running-induced neurogenesis reduces CA1 perineuronal net density without substantial temporal delay" 
        journal="Molecular Brain" 
        page="2024;17(1):64" 
        pmid="39223568" 
        doi="10.1186/s13041-024-01138-x"        
%}
{% include publication 
        authors="Ramkumar, R., Edge-Partington, M., *Terstege, D.J.*, Adigun, K., Ren, Y., Khan, N.S., Rouhi, N., Jamani, N.F., Tsutsui, M., Epp, J.R., Sargin, D."
        title="Long-term impact of early life stress on serotonin connectivity" 
        journal="Biological Psychiatry" 
        page="2024;96(4):287-299" 
        pmid="38316332" 
        doi="10.1016/j.biopsych.2024.01.024"        
%}
{% include publication 
        authors="*Terstege, D.J.*<sup>&#x2a;</sup>, Epp, J.R."
        title="PAW, a cost-effective and open-source alternative to commercial rodent running wheels" 
        journal="HardwareX" 
        page="2024;17(1):e00499" 
        pmid="38204596" 
        doi="10.1016/j.ohx.2023.e00499"        
%}


## 2023
{% include publication 
        authors="*Terstege, D.J.*<sup>&#x2a;</sup>, Dawson, M., Jamani, N.F., Tsutsui, M., Epp, J.R., Sargin, D."
        title="Protocol for the integration of fiber photometry and social behavior in rodent models" 
        journal="Star Protocols" 
        page="2023;4(4):102689" 
        pmid="37979176" 
        doi="10.1016/j.xpro.2023.102689"
%}

{% include publication 
        authors="*Terstege, D.J.*, Epp, J.R."
        title="Parvalbumin as a sex-specific target in Alzheimer’s disease research – A mini-review" 
        journal="Neuroscience and Biobehavioral Reviews" 
        page="2023;153(1):105370" 
        pmid="37619647" 
        doi="10.1016/j.neubiorev.2023.105370"
%}
{% include publication 
        authors="Dawson, M., *Terstege, D.J.*, Jamani, N., Pavlov, D., Tsutsui, M., Bugescu, R., Epp, J.R., Leinninger, G.M., Sargin, D."
        title="Sex-dependent roles of hypocretin/orexin neurons in social behavior" 
        journal="Cell Reports" 
        page="2023;42(7):112815" 
        pmid="37459234" 
        doi="10.1016/j.celrep.2023.112815"
%}


## 2022

{% include publication 
        authors="*Terstege, D.J.*, Epp, J.R."
        title="Network neuroscience untethered: brain-wide immediate early gene expression for the analysis of functional connectivity in freely behaving animals" 
        journal="Biology"
        page="2022;12(1):34" 
        pmid="36671727" 
        doi="10.3390/biology12010034"
%}

{% include publication 
        authors="Moretti, J.<sup>&#8224;</sup>, *Terstege, D.J.*<sup>&#8224;</sup>, Poh, E.Z., Epp, J.R., Rodger, J."
        title="Low intensity repetitive transcranial magnetic stimulation modulates brain-wide functional connectivity to promote anti-correlated c-Fos expression" 
        journal="Scientific Reports"
        page="2022;12(1):20571" 
        pmid="36446821" 
        doi="10.1038/s41598-022-24934-8"
%}
{% include publication 
        authors="*Terstege, D.J.*, Durante, I.M., Epp, J.R."
        title="Brain-wide neuronal activation and functional connectivity are modulated by prior exposure to repetitive learning episodes" 
        journal="Frontiers in Behavioral Neuroscience"
        page="2022;16(1):907707" 
        pmid="36160680" 
        doi="10.3389/fnbeh.2022.907707"
%}
{% include publication 
        authors="*Terstege, D.J.*, Addo-Osafo, K., Teskey, G.C., Epp, J.R."
        title="New neurons in old brains: implications of age in the analysis of neurogenesis in post-mortem tissue" 
        journal="Molecular Brain"
        page="2022;15(1):38" 
        pmid="35501905" 
        doi="10.1186/s13041-022-00926-7"
%}
{% include publication 
        authors="Evans, A.<sup>&#8224;</sup>, *Terstege, D.J.*<sup>&#8224;</sup>, Scott, G.A., Tsutsui, M., Epp, J.R."
        title="Neurogenesis mediated plasticity is associated with reduced neuronal activity in CA1 during context fear memory retrieval" 
        journal="Scientific Reports"
        page="2022;12(1):7016" 
        pmid="35488117" 
        doi="10.1038/s41598-022-10947-w"
%}
{% include publication 
        authors="*Terstege, D.J.*, Oboh, D.O., Epp, J.R."
        title="FASTMAP: Open-Source Flexible Atlas Segmentation Tool for Multi-Area Processing of Biological Images" 
        journal="eNeuro"
        page="2022;9(2):ENEURO.0325-21.2022" 
        pmid="35228311" 
        doi="10.1523/ENEURO.0325-21.2022"
%}

## 2021

{% include publication 
        authors="Scott, G.A., *Terstege, D.J.*, Roebuck, A.J., Gorzo, K.A., Vu, A.P., Howland, J.G., Epp, J.R."
        title="Adult neurogenesis mediated forgetting of multiple types of memory in the rat" 
        journal="Molecular Brain"
        page="2021;14(1):97" 
        pmid="34174906" 
        doi="10.1186/s13041-021-00808-4"
%}

{% include publication 
        authors="*Terstege, D.J.*, MacDonald, D.S., Tasker, R.A."
        title="Standardised ginseng extract G115® potentiates the antidepressant-like properties of fluoxetine in the forced swim test" 
        journal="Acta Neuropsychiatrica"
        page="2021;33(3):141-147" 
        pmid="33478610" 
        doi="10.1017/neu.2021.2"
%}

## 2020

{% include publication 
        authors="Scott, G.A.<sup>&#8224;</sup>, *Terstege, D.J.*<sup>&#8224;</sup>, Vu, A.P.<sup>&#8224;</sup>, Law, S., Evans, A., Epp, J.R. "
        title="Disrupted neurogenesis in germ-free mice: Effects of age and sex" 
        journal="Frontiers in Cell and Developmental Biology"
        page="2020;8(1):407" 
        pmid="32548122" 
        doi="10.3389/fcell.2020.00407"
%}
